...
class Note:
    def __init__(self, tone=0, semitones=0):
        self.tone = tone
        self.semitones = semitones

    def __add__(self, other):
        if isinstance(other, int):
            return Note(tone=self.tone + other, semitones=self.semitones)
        elif isinstance(other, Note):
            return Note(tone=self.tone + other.tone, semitones=self.semitones + other.semitones)
        else:
